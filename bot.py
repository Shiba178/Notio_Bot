...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Запрос к ИИ, чтобы определить намерение пользователя
    ...
    try:
        if action == "создать_событие":
            ...
            await update.message.reply_text(f"✅ Событие '{event_name}' создано на {event_date} с напоминанием за {remind_before} часов.")
        elif action == "показать_события":
            days = int(details.get("дней", 7))
            events = get_upcoming_events(user_id, days)
            if events:
                msg = "📅 Ваши планы:\n" + "\n".join([f"— {e['event_name']} — {e['event_date']}" for e in events])
            else:
